...
static int		divide_each_line(char **line, char **save, char *buf)
{
	...
	inputted_line = *line;
	if (!(tmp = ft_substr(buf, 0, eol_ptr - buf)))
		return (ERROR);
	*line = ft_strjoin(*line, tmp);
	...
	if (!(*line))
		return (ERROR);
	if (!(*save = ft_substr(eol_ptr + 1, 0, ft_strlen(eol_ptr + 1))))
		return (ERROR);
	return (SUCCESS);
}
...
static int		read_fd(int fd, char **line, char **char_buf)
{
	...
	if (!(buf_for_read = malloc(BUFFER_SIZE + 1)))
		return (ERROR);
	while (status_code == READ_CONTINUE && (read_size = read(fd, buf_for_read,
																BUFFER_SIZE)))
	{
